...
# 
# Script to compare Paul's solution (180deg rotations) to the original one (90deg rotation).
# Both the solutions are used to virtually solve the same (random generated) Rubik's cube status.
#
...
# The solver solutionis sent to the robot solver: The robot solver estimates the servos solving time.
# 
...
def imports(plot):
    global np, math, time, rm, rmo, servo, dt, os, path, pathlib
    ...
    import Cubotino_T_moves_orig as rmo           # custom library, traslates the cuber solution string in robot movements string
...
def introduction():
    row = "#"*103                                 # string of characters used as separator
    print(row)                                    # separator is printed to the terminal
    print(" Script comparing Paul's solution (180deg rotations) performance vs the original one (90deg rotation).")
    print(" Both the solutions are used to virtually solve thesame (random generated) Rubik's cube status.")
    print(" Kociemba solver is used to generate the rando cune status, and to return a solution for it.")
    print(" The solution is sent to the robot solver (used Cubotino_T_moves from Paul and the original one).")
    print(" The robot solver estimates the servos solving time.")
...
def plot_interpreted_colors(wait, cube_status, test, test_type, startup=False, kill=False):
    ...
    if startup:
        ...
        if test_type == 180:
            ...
            text = ''
        cv2.putText(sketch, text, (x_start+int(6.6*d), y_start+int(0.6*d)), font, fontScale*1.2,(0,0,0),lineType)
...
def cube_solution(cube_string, printout, informative):
    ...
    _, robot_moves1, total_moves1, opt1 = rm.robot_required_moves(s, solution_Text, simulation=simulation, informative=informative)
    est_time1 = servo.estimate_time(robot_moves1, timer, slow_time=0)   # estimated time for the robot moves in argument
    
    _, robot_moves2, total_moves2, opt2 = rmo.robot_required_moves(s, solution_Text, simulation=simulation, informative=informative)
    est_time2 = servo.estimate_time(robot_moves2, timer, slow_time=0)   # estimated time for the robot moves in argument
    
    depth1 = depth2 = len(s)//3                   # cube status depth
    ...
    return s, solution_Text, robot_moves1, total_moves1, est_time1, opt1, depth1, robot_moves2, total_moves2, est_time2, opt2, depth2
...
def test_random_permutations(runs, cs, timer, plot, debug, printout):
    ...
    # cs means cube_status
    start = time.time()                           # initial time reference 
    
    if len(cs.strip().replace(" ","")) == 54:     # case the script was launched with a cube status as argument
    ...
    if plot:                                      # case plot variable is set True (sketch with the cube status on screen)
        if random_status:                         # case random_cube is set True (random cube status generation)
            ...
            t2 = 30                               # variable t2 in ms(plot time for cube status while moving the cube)
    ...
    test_ok_number = 0                            # initialized counters
    cube_depth = []                               # initialized empty lists to store values for stats (cube depth is the same)
    
    opt1_1, opt2_1 = 0,0                          # initialized counters, for optimizations type1 and type2, on 180deg solution
    tot_robot_moves1, estimated_time1, opt1_list1, opt2_list1  = [], [], [], []  # initialized empty lists to store values for stats, on 180deg solution
    
    opt1_2, opt2_2 = 0,0                          # initialized counters, for optimizations type1 and type2, on 90deg solution
    tot_robot_moves2, estimated_time2, opt1_list2, opt2_list2  = [], [], [], []   # initialized empty lists to store values for stats, on 90deg solution
    ...
    for test in range(1,runs+1):                  # iteration over the test runs
        if runs>1:
            if not debug and not printout:        # case debug and printout are set False
                print(f"  Test: {test:,d} (of {runs:,d})", end='\r', flush=True) # feedback is write and overwrite to terminal
        ...
        if random_status:                         # case random_cube is set True (random cube status generation)
            cs = scramble()                       # a random cube_status is generated

        if printout:                              # case printout is set True
            if random_status:                     # case the cube status is random generated
                print("Random cube_status generated:", cs)  # feedback is printed to terminal
            else:                                 # case the cube status is entered by user
                print("Entered cube_status:", cs) # feedback is printed to terminal
...
#         solution, solution_Text, robot_moves, total_robot_moves, est_time, opt, depth = a,b,c,d,e,f,g
        ...
        a,b,c,d,e,f,g,h,i,k,l,m = cube_solution(cs, printout,informative=informative) # Kociemba solver is called to have the solution string
        s, solution_Text, robot_moves1, total_moves1, est_time1, opt_1, depth1, robot_moves2, total_moves2, est_time2, opt_2, depth2 = a,b,c,d,e,f,g,h,i,k,l,m
        
        for i, data in enumerate(((robot_moves1, total_moves1, est_time1, opt_1, depth1), (robot_moves2, total_moves2, est_time2, opt_2, depth2))):
            
            robot_moves, total_robot_moves, est_time, opt, depth = data
            cube_status = cs
            
            if solution_Text != 'Error':          # case no errors returned by the Kociemba solver
                if i == 0:
                    test_type = 180
                    cube_depth.append(depth)      # cube_status depth is appended to cube_depth list (same depth on both analysis)
                    tot_robot_moves1.append(total_robot_moves) # quantity of robot movements is appended to tot_robot_moves
                    estimated_time1.append(est_time)  # the estimated time for this run is appended to estimated_time list
                    opt1_list1.append(opt[0])     # optimization type1 usage is appended
                    opt2_list1.append(opt[1])     # oprimization type2 usage is appended
                    opt1_1 += opt[0]              # counter is incremented by the optimization type 1 that were applied
                    opt2_1 += opt[1]              # counter is incremented by the optimization type 2 that were applied
                elif i == 1:
                    test_type = 90
                    tot_robot_moves2.append(total_robot_moves) # quantity of robot movements is appended to tot_robot_moves
                    estimated_time2.append(est_time)  # the estimated time for this run is appended to estimated_time list
                    opt1_list2.append(opt[0])     # optimization type1 usage is appended
                    opt2_list2.append(opt[1])     # optimization type2 usage is appended
                    opt1_2 += opt[0]              # counter is incremented by the optimization type 1 that were applied
                    opt2_2 += opt[1]              # counter is incremented by the optimization type 2 that were applied
                ...
                if plot:                          # case plot variable is set True (sketch with the cube status on screen)
                    ...
                    plot_interpreted_colors(show_ms, cube_status, test, test_type, startup=True) # initial cube status is plot to the screen
                ...
                for i in range(0, len(robot_moves),2): # iteration over the robot movements
                    ...
                    if move_type == 'F':          # case the robot move is F (flip)
                        for i in range(int(direction)):  # iteration over the quantity of flips
                            ...
                            if plot:              # case plot variable is set True (sketch with the cube status on screen)
                                plot_interpreted_colors(show_ms, cube_status, test, test_type)  # the new cube status is plot to the screen
                ...
                if plot:                          # case plot variable is set True (sketch with the cube status on screen)
                    ...
                    if test != runs:              # case the current test is not the last one
                        plot_interpreted_colors(show_ms, cube_status, test, test_type)  # the final cube status is plot to the screen
                    else:                         # case the current test is the last one
                        # the final cube status is plot to the screen, with kill instruction
                        plot_interpreted_colors(show_ms, cube_status, test, test_type, kill=True)  
    ...
    if solution_Text != 'Error':                  # case no errors returned by the Kociemba solver
        print('\n')                               # print empty line as separation
        print(row)                                # print a separation line
        print('\n')                               # print empty line as separation
        if debug:                                 # casedebug is set True
            print()                               # print empty line as separation
            print(row)                            # print a separation line
            print(row)                            # print a separation line
        
        if runs>1:                                # case were tested more than one run
            print(f"Tested {runs} random cube's status with {2*runs-test_ok_number} failures")
        ...
        if runs>1:                                # case tested more than a single test (runs)
            ...
            # stats for Pauls solutions (180deg rotations)
            total_moves1 = np.array(tot_robot_moves1)     # an array is generated from the list of tot_robot_moves
            est_time1 = np.array(estimated_time1)         # an array is generated from the list of estimated servos time
            avg_moves1 = round(np.mean(total_moves1),1)   # average of robot movements
            avg_servo_time1 = round(np.mean(est_time1),1) # average servos time
            std_servo_time1 = round(np.std(est_time1),3)  # standard deviation of the average value
            min_servo_time1 = round(np.min(est_time1),1)  # minimum servos time
            max_servo_time1 = round(np.max(est_time1),1)  # maximum servos time
            opt1_list1 = np.array(opt1_list1)             # an array is generated from the list of optimizations type1
            opt2_list1 = np.array(opt2_list1)             # an array is generated from the list of optimizations type2
            
            # stats for original solutions (90deg rotations)
            total_moves2 = np.array(tot_robot_moves2)     # an array is generated from the list of tot_robot_moves
            est_time2 = np.array(estimated_time2)         # an array is generated from the list of estimated servos time
            avg_moves2 = round(np.mean(total_moves2),1)   # average of robot movements
            avg_servo_time2 = round(np.mean(est_time2),1) # average servos time
            std_servo_time2 = round(np.std(est_time2),3)  # standard deviation of the average value
            min_servo_time2 = round(np.min(est_time2),1)  # minimum servos time
            max_servo_time2 = round(np.max(est_time2),1)  # maximum servos time
            opt1_list2 = np.array(opt1_list2)             # an array is generated from the list of optimizations type1
            opt2_list2 = np.array(opt2_list2)             # an array is generated from the list of optimizations type2
            
            # prints some stats to the termional
            print()
            print("Paul's solution (180deg rotations) vs the original solution (90deg rotations):")
            print(f"Average quantity of robot movements:     {avg_moves1}\t{avg_moves2}")
            print(f"Average estimated solving time (servos): {avg_servo_time1}\t{avg_servo_time2}")
            print(f"std on estimated solving time (servos):  {std_servo_time1}\t{std_servo_time2}")
            print(f"Min estimated solving time (servos):     {min_servo_time1}\t{min_servo_time2}")
            print(f"Max estimated solving time (servos):     {max_servo_time1}\t{max_servo_time2}")
            print(f"Optimization type 1 being used:          {opt1_1}\t{opt1_2}")
            print(f"Optimization type 2 being used:          {opt2_1}\t{opt2_2}")
            print()
            ...
            headers = 'Cube_depth\tTot_robot_moves1\tEstimated_time1\tOpt1\tOpt2\tTot_robot_moves2\tEstimated_time2\tOpt1\tOpt2'
            np.savetxt(fname, np.c_[cube_depth, total_moves1, est_time1, opt1_list1, opt2_list1, total_moves2, est_time2, opt1_list2, opt2_list2],
                       fmt='%10.1f', delimiter='\t', comments='', header=headers,)   # data is saved to the text file
